import React, { useState, SVGProps, forwardRef, ReactElement, InputHTMLAttributes, ForwardedRef } from 'react';
...
const Input = (
  ...
  const uid = 'componente-' + Math.floor(Math.random() * 999999);
      ...
      <div className="relative bg-slate-300 bg-opacity-50 dark:bg-slate-800  h-[60px] rounded-tr-md rounded-tl-md shadow w-full">
        ...
        {props.type === 'password' && (
          <button
            ...
            className={`${hasError ? 'btn-ghost-red' : 'btn-ghost-primary'} btn-icon  absolute right-3 top-[13px]`}
          >
            {show ? (
              <IconEyeSlash className={`${hasError ? 'text-red-600 dark:text-red-400' : 'text-primary-500'}`} />
            ) : (
              <IconEye className={`${hasError ? 'text-red-600 dark:text-red-400' : 'text-primary-500'}`} />
            )}
          </button>
        )}

        {typeof Icon === 'function' && (
          <button className="btn-icon btn-ghost-primary absolute right-3 top-[13px]">{<Icon />}</button>
        )}
